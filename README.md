...
- **例文の和訳自動取得**: MyMemory API（無料・オンライン時）。
- **ミニ辞書検索**: 単語帳の検索窓に英単語を入れると未登録語でも内蔵辞書から意味＋追加ボタン。
- **苦手フィルタ**（lapses≥2）／**今日の実績**表示／**起動時に復習タブ**（設定でオフ可）
- **iPhone向け**: スワイプ採点（→普通・←もう一度）、アプリバッジ(setAppBadge)、apple-touch-icon正方形化
